...
export default function AuthEntry({ onAuthenticated }: { onAuthenticated: (session: PrototypeSession) => void }) {
  ...
  const reviewLocal = (event: FormEvent) => {
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-panel" aria-label="Sign in to OpenKubes">
      <div className="auth-card">
        ...
        {stage === 'local' && <>
          <button className="auth-back" type="button" onClick={reset}>← Back to federated sign-in</button>
          <div className="auth-heading"><span className="auth-kicker auth-kicker-warning">Exceptional access</span><h1>Bootstrap / break-glass</h1><p>Use a local account only when federation is not yet configured or unavailable.</p></div>
          <div className="auth-warning"><AuthIcon name="shield"/><div><strong>This is not the everyday login path</strong><p>A production attempt requires rate limiting, MFA where available, short expiry, immutable audit evidence, and an operational reason.</p></div></div>
          <form className="auth-local-form" onSubmit={reviewLocal}>
            <label><span>Local username</span><input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" placeholder="bootstrap-admin"/></label>
            <label><span>Password</span><span className="auth-password"><input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" placeholder="Enter local password"/><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Hide password' : 'Show password'}><AuthIcon name="eye" size={17}/></button></span></label>
            <label><span>Operational reason</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Why is federated access unavailable?" rows={3}/><small>Minimum 12 characters · intended for immutable audit evidence</small></label>
            ...
            <button className="auth-primary" type="submit" disabled={!localReady}>Review local session <AuthIcon name="arrow"/></button>
          </form>
          <p className="auth-simulation">Prototype hint: username <code>blocked</code> demonstrates a denied attempt.</p>
        </>}
